...
const SaleFilter: React.FC<SaleFilterProps> = ({ sales, sale, setSale }) => {
  const [isOpen, setIsOpen] = useState(false);

  const toggleDropdown = () => setIsOpen(!isOpen);

  const handleSaleSelect = (sale: string) => {
    setSale([sale]);
  };
        ...
        <div className="w-full text-sm text-kaiglo_grey-base transition duration-500 ease-in-out max-h-80 overflow-hidden overflow-y-auto">
          ...
          >
            {sales.map((item) => (
              ...
              >
                {capitalizeFirstLetterOfEachWord(item.replace(/_/g, " "))}
              </p>
            ))}
          </div>
        </div>
