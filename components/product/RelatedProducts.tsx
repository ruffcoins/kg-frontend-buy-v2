...
const RelatedProducts = ({ productId }: { productId: string }) => {
    ...
    if (status === "error") {
        return <ErrorComponent message="Failed to load related products." action={refetchRelatedProducts} />;
    }
    ...
    return (
        <div className="lg:px-8 px-4 space-y-5 py-10">
            ...
            <div ref={ref}>{/* This empty div acts as a sentinel for the IntersectionObserver */}</div>
        </div>
    );
};
