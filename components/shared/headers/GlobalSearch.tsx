...
const GlobalSearch = ({ customTopClassname }: { customTopClassname?: string }) => {
    const [searchTerm, setSearchTerm] = useState("");
    const { products, isFetching } = useSearchProducts(searchTerm);
    const [isResultsVisible, setIsResultsVisible] = useState(false);
    const [recentSearches, setRecentSearches] = useState<Array<{ id: string; term: string }>>([]);
    ...
    return (
        <div className="w-full relative">
            <form onSubmit={handleSubmit} className="relative flex items-center w-full">
                ...
                />
                <Button type="submit" variant="accent" className="absolute w-8 h-8 p-0 rounded-full right-2">
                    <Image src={Search} alt="search icon" className="w-4 h-4" width={16} height={16} />
                </Button>
            </form>
                                                ...
                                                <li className="flex items-center justify-between py-3 px-6 hover:bg-gray-100 cursor-pointer">
                                                    <span className="flex items-start space-x-2">
                                                        <Image
                                                            ...
                                                            height={56}
                                                        />
                                                        <span className="hidden lg:block">{result.name}</span>
