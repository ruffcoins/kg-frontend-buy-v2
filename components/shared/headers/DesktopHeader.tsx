...
const GlobalSearch = dynamic(() => import("@/components/shared/headers/GlobalSearch"), {
    ssr: false,
});
    ...
    loading: () => (
        <div className="flex items-center space-x-4">
            {Array.from({ length: 7 }).map((_, index) => (
                <div key={index} className="rounded-full w-20 h-8 bg-slate-200 animate-pulse"></div>
            ))}
        </div>
    ),
});
                    ...
                    <div className="flex items-center xl:space-x-4 lg:space-x-2">
                        ...
                        >
                            <span className="bg-kaiglo_brand-base rounded-full min-w-8 min-h-8 flex items-center justify-center">
                                <Image src={Menu} alt="menu icon" className="min-w-4 h-min-h-4" />
                            </span>
                    ...
                    {showCallToOrder && (
                        <div className="xl:flex hidden items-center space-x-1">
                            ...
                            <div className="flex-col">
                                <p className="text-[13px] 2xl:text-sm font-bold">Call to place order</p>
